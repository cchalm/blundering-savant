	...
	conversation := newTestConversation(t, toolRegistry, []ai.ConversationTurn{
		{
			...
			Response: github.Ptr(createResponse(t, anthropic.NewAssistantMessage(
				...
				anthropic.NewToolUseBlock("tool_use_id_1", TextEditorInput{Command: "view", Path: ""}, textEditorTool.Name),
				anthropic.NewToolUseBlock("tool_use_id_2", TextEditorInput{Command: "view", Path: "docs/"}, textEditorTool.Name),
				anthropic.NewToolUseBlock("tool_use_id_3", TextEditorInput{Command: "view", Path: "docs/SERVER_README.md"}, textEditorTool.Name),
			))),
		},
		{
			UserMessage: anthropic.NewUserMessage(
				anthropic.NewToolResultBlock("tool_use_id_1", "Directory contents of : README.md go.mod go.sum docs/ app/ pkg/", false),
				anthropic.NewToolResultBlock("tool_use_id_2", "Directory contents of docs: SERVER_README.md CLIENT_README.md", false),
...
func TestBotReactsToCommentsUsingParallelToolCalls(t *testing.T) {
	...
	tsk.IssueCommentsRequiringResponses = tsk.IssueComments
